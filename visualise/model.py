...
from visualise.config import Config      # import the objects defined in ther scripts
from visualise.dataset import Dataset
from visualise.discriminator import Discriminator
from visualise.generator import Generator
from visualise.model_util import batch_align_by_pelvis, batch_compute_similarity_transform, batch_rodrigues
...
class ExceptionHandlingIterator:
    ...
    def __iter__(self):
        return self     # this makes the class iterable
...
class Model:
    ...
    def __init__(self, display_config=True):
        ...
        print('******************\n', self.config.LOG_DIR)      # added by zzx
    ...
    def _build_model(self):
        ...
        if not self.config.ENCODER_ONLY:    # if false, adversarial will be trained in prior
    ...
    @tf.function
    def _train_step(self, images, kp2d, kp3d, has3d, theta):    # define the training process (including the loss calculation)
        tf.keras.backend.set_learning_phase(1)
        batch_size = images.shape[0]

        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            generator_outputs = self.generator(images, training=True)
            #### IMPORTANT: only use last computed theta (from iterative feedback loop)
            ...
            if self.config.USE_3D:    # USE_3D is default set as true
                ...
                """Calculating pose and shape loss basically makes no sense
                    due to missing paired 3d and mesh ground truth data.
            ...
            gen_disc_loss = tf.reduce_mean(tf.reduce_sum((fake_disc_output - 1) ** 2, axis=1))  #computes the mean of the elements across dimensions of a tensor
    ...
    def _log_train(self, epoch):
        ...
        with self.summary_writer.as_default():  # so this step is to store the training parameters  # zzx: the tf.summary.scalar writes the simple numeric value onto the default summary writer for later tensorboard analysis.
    ...
    def detect(self, image):
        ...
        if len(tf.shape(image)) != 4:
            image = tf.expand_dims(image, 0) # add a dimension to the tensor on the first dimension
        # pdb.set_trace()   # set a break point
        result = self.generator(image, training=False)

        vertices_pred, kp2d_pred, kp3d_pred, pose_pred, shape_pred, cam_pred = result[-1]   # the last element of the 
        result_dict = {
