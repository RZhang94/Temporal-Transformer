...
class Regressor(tf.keras.Model):
    ...
    def __init__(self):   # the layers and configuration imported from other scripts
    ...
    def call(self, inputs, **kwargs):   # the generating steps
        batch_size = inputs.shape[0] or self.config.BATCH_SIZE
        shape = (batch_size, 2048)      # this 2048 probabilty defines the input image size
    ...
    def _fc_blocks(self, inputs, **kwargs):   # forward function
        x = self.fc_one(inputs, **kwargs)     # kwargs = key word argument
        x = tf.nn.relu(x)
...
class Generator(tf.keras.Model):
    ...
    def _compute_output(self, theta, **kwargs):
        cams = theta[:, :self.config.NUM_CAMERA_PARAMS]     # 在NUM_CAMERA_PARAMS列之前的是cams parameters
        pose_and_shape = theta[:, self.config.NUM_CAMERA_PARAMS:]   # behind that row are pose and shapes
        vertices, joints_3d, rotations = self.smpl(pose_and_shape, **kwargs)    # transform the pose and shape parameter to SMPL vertices
